...
function pickRandomVideo(kind: 'win' | 'lose'): string {
 ...
 */
export function maybeShowPerfectVideo(correctCount: number, wrongCount: number, totalRounds: number): void {
    const overlay = document.getElementById('perfect-video-overlay');
    const video = document.getElementById('perfect-video') as HTMLVideoElement | null;
    if (!overlay || !video || totalRounds <= 0) return;

    let kind: 'win' | 'lose' | null = null;
    if (correctCount === totalRounds) kind = 'win';
    else if (wrongCount === totalRounds) kind = 'lose';

    if (!kind) return; // نتيجة مختلطة، مفيش فيديو
