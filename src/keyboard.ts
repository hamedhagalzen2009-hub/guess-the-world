...
const KEY_BASE =
    'vk-key flex items-center justify-center flex-1 h-13 rounded-sm border-2 border-black border-b-4 ' +
    'bg-[#eee] dark:bg-neutral-800 text-black dark:text-neutral-100 font-bold uppercase text-base leading-none';

const DEL_BASE =
    'vk-key flex items-center justify-center h-13 rounded-sm border-2 border-black border-b-4 ' +
...
const letterStatus: Record<string, KeyStatus> = {};

/* ---------- input helpers ---------- */

function activeRow(): HTMLElement | null {
    const rows = document.querySelectorAll<HTMLElement>('.inputs > div');
    for (const row of Array.from(rows)) {
        if (!row.classList.contains('disabled')) return row;
    }
    return null;
}
...
function rowInputs(): HTMLInputElement[] {
    const row = activeRow();
    if (!row) return [];
    return Array.from(row.querySelectorAll<HTMLInputElement>('input'));
}

function focusedIndex(inputs: HTMLInputElement[]): number {
...
function makeKey(label: string, onClick: () => void, cls: string, letterKey?: string): HTMLButtonElement {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = cls;
    btn.textContent = label;
    if (letterKey) btn.dataset.key = letterKey;
...
function applyColors(): void {
    if (!container) return;
    container.querySelectorAll<HTMLElement>('[data-key]').forEach((btn) => {
        const status = letterStatus[btn.dataset.key ?? ''];
        if (status) {
...
export function initKeyboard(_handlers?: { onEnter?: () => void }): void {
    container = document.getElementById('virtual-keyboard');
    if (container && !currentLang) render('en');
}

/** Pick the layout automatically from the current word (Arabic letters -> Arabic layout). */
export function setKeyboardForWord(word: string): void {
    if (!container) container = document.getElementById('virtual-keyboard');
    const lang: 'en' | 'ar' = ARABIC_RE.test(word) ? 'ar' : 'en';
    if (lang !== currentLang) render(lang);
}

/** Merge the statuses computed by handelCheck(), keeping the best per letter. */
export function updateKeyboardColors(results: { letter: string; status: string }[]): void {
    const rank: Record<string, number> = { lose: 1, mud: 2, win: 3 };
    results.forEach(({ letter, status }) => {
        if (!letter || !status) return;
        const key = letter.toLowerCase();
        const prev = letterStatus[key];
        if (!prev || (rank[status] ?? 0) > (rank[prev] ?? 0)) {
            letterStatus[key] = status as KeyStatus;
        }
    });
    applyColors();
}
...
export function resetKeyboardColors(): void {
